...
1. **Pre-commit hooks** — linters, formatters, type checks (none yet — no pre-commit hooks, linters, or build tools configured; update this when you introduce a build system).
2. **Slice unit tests** — each slice's tests run during `/implement-slice`.
